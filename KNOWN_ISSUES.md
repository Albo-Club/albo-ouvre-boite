...
three self-refreshing channels". `guidelines.md` tracks upstream (via
`npx convex ai-files update`) and outranks skills by our own rule; the Convex
MCP reads the live deployment and cannot go stale.
...
## …and the Convex CLI puts them straight back — `convex.json` stops it

Pruning them from `skills-lock.json` was only half the job. The Convex CLI
installs those same skills on its own, so the section above was a decision the
tooling quietly reversed on the next `convex dev`.

### What it does

`convex dev` calls `attemptSetupAiFiles()` (`cli/configure.js`, and
`cli/lib/localDeployment/anonymous.js` for anonymous deployments). That path
runs `installAiFiles()`, which does four things in a row — and only the first
is the one we want:

| step                      | writes                                            | verdict |
| ------------------------- | ------------------------------------------------- | ------- |
| `installGuidelinesFile()` | `convex/_generated/ai/guidelines.md`              | **keep** — channel #1 in `CLAUDE.md` |
| `applyAgentsMdSection()`  | the managed block in `AGENTS.md`                  | keep — that block *is* our `AGENTS.md` |
| `applyClaudeMdSection()`  | a `<!-- convex-ai-start -->` block in `CLAUDE.md` | harmless once skills are off |
| `installSkills()`         | `.agents/skills/convex-*` ×32, their `.claude/skills/` symlinks, **and entries in our `skills-lock.json`** | **block** |

The last one is what breaks CI. It shells out to `npx skills@latest add
get-convex/agent-skills`, which writes lock entries with no `trackingRef` and
no `pinnedRef` — so `sync-skills --verify` exits 2 and the `skills-verify` job
goes red on a tree nobody edited by hand. It also overwrites
`convex-create-component/references/advanced-patterns.md`, the one Convex skill
we deliberately kept.

Worse than a prompt: `attemptSetupAiFiles()` skips the "Set up Convex AI
files?" question entirely when `hasAiState()` is true, and
`convex/_generated/ai/ai-files.state.json` is **tracked in this repo**. A fresh
clone therefore installs all 32 without asking.

### The fix, and why not the obvious one

```jsonc
// convex.json
{ "aiFiles": { "skills": { "agents": [] } } }
```

`configuredSkillAgents()` reads `aiFiles.skills.agents` and defaults to
`['claude-code', 'codex']`; `installSkills()` returns immediately on an empty
list, *before* the `npx skills add`. Everything above it in the table still
runs.

**Do not reach for `npx convex ai-files disable` instead.** It writes
`aiFiles.enabled: false`, and `isAiFilesDisabled()` is checked at the *top* of
`attemptSetupAiFiles()` and of `checkVersionAndAiFilesStaleness()` — so it also
kills the guidelines install and the "your AI files are out of date" nudge.
That trades a CI break for a silently rotting channel #1. Same objection to
`disableStalenessMessage: true`, which `isAiFilesDisabled()` treats as
`enabled: false`.

`convex.json` is committed, so the guard is in place *before* the first
`convex dev` — there is nothing for `scripts/setup.mjs` to clean up afterwards,
which is why the wizard has no step for this. It also holds for every later
invocation, including a `npx convex ai-files install` or `update` typed by
hand: with the config present those commands refresh the guidelines and leave
the skills alone. The point is not "never run that command", it is "make it
harmless".

### If a derived project already has them

A project created before this landed has the 32 skills and the polluted lock.
Add `convex.json`, then:

```bash
npx skills remove $(ls .agents/skills | grep '^convex-' | grep -v '^convex-create-component$') --yes
git checkout skills-lock.json .agents/skills/convex-create-component
pnpm run sync:skills          # prunes the orphaned .claude/skills symlinks
pnpm run sync:skills:verify   # must print "Vendored skills match…"
```

## A vendored skill can ship an upstream bug — don't hand-fix it

`convex-create-component/references/advanced-patterns.md` currently contains,
at the pin bumped in v0.4.0, an example whose handler reads `args.id` while its
signature only binds `ctx`:

```ts
export const getNotification = internalQuery({
  args: { id: schema.id("notifications") },
  returns: v.nullable(vNotification),
  handler: async (ctx) => {                       // ← `args` never bound
    const notification = await ctx.db.get("notifications", args.id)
```

That is upstream's bug (`get-convex/agent-skills`), not a vendoring accident.
**Leave it.** `computedHash` covers the vendored bytes, so a hand edit makes
`sync-skills --verify` exit 2 and turns `skills-verify` red — the gate is doing
its job, and patching the lock to match the edit would be the one thing
`CLAUDE.md` says never to do. The general rule: our copy is a faithful mirror,
and mirrors reproduce warts. Report it upstream, or override the guidance here
in `CLAUDE.md` where our own words outrank a vendored file.
