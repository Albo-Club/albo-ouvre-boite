...
### Follow-up — `isSignedOut` must not fire on a transient `/get-session`

#### Symptom (prod)

Opening a new tab to `/app/*`, or returning after the app sat idle (the
"every morning" report), redirects to `/login` even though the session
cookie (`__Secure-albo.session_token`, 7-day) is valid and a manual
`GET /api/auth/get-session` returns **200 with the session**. Confirmed live
on `louvre-boite.alboteam.com`: get-session 200, yet the guard bounced to
`/login`. Only reproduces from a *cold* state — once the BA cookieCache
(`__Secure-albo.session_data`, 5-min) is warm, `useSession()` resolves
instantly and the redirect never happens, which is why it hid in testing.

#### Root cause

`useAuthState()`'s `isSignedOut` was `!baPending && !hasBaUser`. Better
Auth's `useSession()` is a nanostore fed by `/get-session`
(`better-auth/dist/client/query.mjs`). On a failed/aborted/cold request —
Vercel function cold start (~600 ms), request aborted during hydration, or a
cross-tab focus refetch (BA refetches on focus + broadcasts via
BroadcastChannel) — the atom settles to `{ data: null, isPending: false }`
with an `error` set. The old `isSignedOut` ignored `error`/`isRefetching`,
so a transient blip read as a real logout and the `/app` guard's
`navigate('/login')` fired before the next (successful) fetch landed.

Phase 3 closed the *Convex-JWT* gap (`hasBaUser && !convexAuth` → loading);
this is the parallel *BA-session-refetch* gap it didn't cover.

#### Rule

- `isSignedOut = !baPending && !baRefetching && !hasBaUser && baError == null`
  — only a positively-settled "no session" counts. A fetch error/abort/cold
  start leaves the guard in `Loading…`, not signed-out; BA refetches on
  focus/interval and recovers.
- The `/app` guard additionally **debounces** the redirect (~1.5 s with
  cleanup): a transient `isSignedOut` clears within a few hundred ms and the
  cleanup cancels the pending `navigate`. Both layers live in
  `src/lib/auth-state.ts` and `src/routes/app/route.tsx`.
- Backend/env are NOT involved here — `SITE_URL`, `APP_ENV`, cookies, and the
  same-domain `/api/auth/*` proxy were all verified healthy. Don't chase env
  config for a "logged out on new tab" report until you've confirmed
  get-session actually returns no session.
