...
2. **Dev tooling** (the `resend@claude-plugins-official` Claude Code plugin,
   ...
   `npx -y resend-mcp` and reads `RESEND_API_KEY` from the **developer's
   shell environment** — not the Convex env, not `.env.local`. Set it in
   your shell profile (`export RESEND_API_KEY=re_…`).
