...
1. `convex/_generated/ai/guidelines.md` — installed by the first `convex dev`
   and refreshed by `npx convex ai-files update`. Required reading before
   non-trivial Convex patterns; **it overrides everything, including upstream
   skills.** Note the refresh is *nudged*, not automatic: later `convex dev`
   runs only print "Your Convex AI files are out of date" — act on it, or this
   channel quietly stops being fresher than the skills it outranks.
...
`KNOWN_ISSUES.md` section first — the gap is usually covered by 1 or 2. Nor by
accident: `convex.json` sets `aiFiles.skills.agents: []` precisely so the
Convex CLI stops reinstalling all 32 of them (and breaking `skills-verify`) on
`convex dev`. Never delete that key, and never swap it for
`ai-files disable` — that switch also takes channel 1 down. Same section of
`KNOWN_ISSUES.md`.
